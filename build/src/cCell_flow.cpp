...
void cCell_flow::init_const()
{
  ...
  double vAe4 = s.Sb * ((p.at("Cle") / (p.at("Cle") + p.at("KCl"))) * (p.at("Na0") / (p.at("Na0") + p.at("KNa"))) *
                        pow(p.at("HCO30") / (p.at("HCO30") + p.at("KB")), 2));
...
void cCell_flow::step()
{
  ...
  double JNaKb =
    NaKbasalfactor * s.Sb * s.aNaK *
    (p.at("r") * pow(p.at("Ke"), 2) * pow(solvec(Na), 3) / (pow(p.at("Ke"), 2) + p.at("alpha1") * pow(solvec(Na), 3)));

  // JNaKa = (1-NaKbasalfactor)*Sa*aNaK*(param.r * Kl^2 * Na^3 ...
  //  / ( Kl^2 + param.alpha1 * Na^3 ) );
  double JNaKa =
    (1 - NaKbasalfactor) * s.Sa * s.aNaK *
    (p.at("r") * pow(solvec(Kl), 2) * pow(solvec(Na), 3) / (pow(solvec(Kl), 2) + p.at("alpha1") * pow(solvec(Na), 3)));
  ...
  double Qb = p.at("B2") * (2 * (solvec(Na) + solvec(K) + solvec(H)) + p.at("CO20") -
                            (p.at("Nae") + p.at("Ke") + p.at("Cle") + p.at("HCO3e")));
  double t =
    p.at("B3") * (2 * (solvec(Nal) + solvec(Kl)) + p.at("Ul") - (p.at("Nae") + p.at("Ke") + p.at("Cle") + p.at("HCO3e")));
  double Qt =
    p.at("B3") * (2 * (solvec(Nal) + solvec(Kl)) + p.at("Ul") - (p.at("Nae") + p.at("Ke") + p.at("Cle") + p.at("HCO3e")));
  double Qtot = Qa + Qt;
  ...
  double JAe4 = s.Sb * s.G4 *
                ((p.at("Cle") / (p.at("Cle") + p.at("KCl"))) * (solvec(Na) / (solvec(Na) + p.at("KNa"))) *
                 pow(solvec(HCO3) / (solvec(HCO3) + p.at("KB")), 2));
  ...
  double JNhe1 = s.Sb * s.G1 *
                 ((p.at("Nae") / (p.at("Nae") + p.at("KNa"))) * (solvec(H) / (p.at("KH") + solvec(H))) -
                  (solvec(Na) / (solvec(Na) + p.at("KNa"))) * (p.at("He") / (p.at("KH") + p.at("He"))));
